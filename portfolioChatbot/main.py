...
def get_completion_from_messages(messages, model='text-davinci-002', temperature=0):
    ...
    response = openai.Completion.create(engine=model, prompt=prompt, temperature=temperature)  # this is the degree of randomness of the model's output
...
def lambda_handler(event, context):
    try:
        ...
        # Return an error response if the body or userInput key is missing

        return {'statusCode': 400, 'headers': {  # Enable CORS for your website domain
    ...
    return {'statusCode': 200, 'headers': {  # Enable CORS for your website domain
        'Content-Type': 'application/json',
        'Access-Control-Allow-Origin': 'https://www.msaad.co.in',
        'Access-Control-Allow-Methods': 'POST, OPTIONS',
        'X-Content-Type-Options': 'nosniff',
